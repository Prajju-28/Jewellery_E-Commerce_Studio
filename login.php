...
if(isset($_POST['Login']))
{
	...
	if($type == 'Admin')
	{
		...
		{
			echo"<script>alert('Failed to Login Check your Email-Id and password');</script>";
		}
	}
	else if($type == 'Users')
	{
		...
		{
				echo"<script>alert('Wrong Id and Password!! Try Again');</script>";
		}
	}		
	else if($type == 'Dealers')
	{
		$qry="SELECT * FROM dealers_register WHERE DEmail ='$email' AND DLnumber='$password'";
		$res=$conn->query($qry);
		if($res->num_rows > 0)
		{
			while($row=$res->fetch_assoc())
			{
				if($row['DEmail']== $email && $row['DLnumber'] == $password)
				{
					$_SESSION['Dealers']=$email;
					if($row['status']!= 'Activated')
					{
						header("location:Otp.php");
					}
      ...
      <div class="row">
        <div class="col-md-6">
          <form action="login.php" method="POST">
				...
				<div class="col-lg-12">
				  <input type="passsword" placeholder="Password" name="Passsword"  required />
				</div>
